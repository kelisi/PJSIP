...
{
    PJ_LOG(3, (THIS_FILE, "[io] create select ioqueue"));
...
pj_ioqueue_poll(pj_ioqueue_t *ioqueue, const pj_time_val *timeout)
{
    ...
    {
#endif
        // PJ_LOG(3, (THIS_FILE,"[io][select] select socket "));
    ...
    for (i = 0; i < event_cnt; ++i)
    {
        ...
        if (processed_cnt < PJ_IOQUEUE_MAX_EVENTS_IN_SINGLE_POLL)
        {
            ...
            {
            case READABLE_EVENT:
                // PJ_LOG(3, (THIS_FILE,"[io][select][socket] dispatch read event"));
                if (ioqueue_dispatch_read_event(ioqueue, event[i].key))
                    ++processed_cnt;
                break;
            case WRITEABLE_EVENT:
                // PJ_LOG(3, (THIS_FILE,"[io][select][socket] dispatch write event"));            
                if (ioqueue_dispatch_write_event(ioqueue, event[i].key))
                    ++processed_cnt;
                break;
            case EXCEPTION_EVENT:
                // PJ_LOG(3, (THIS_FILE,"[io][select][socket] dispatch exception event"));            
