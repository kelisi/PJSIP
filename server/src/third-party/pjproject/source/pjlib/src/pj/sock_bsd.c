 ...
 */
PJ_DEF(pj_uint16_t)
pj_htons(pj_uint16_t hostshort)
{
 ...
 */
PJ_DEF(char *)
pj_inet_ntoa(pj_in_addr inaddr)
{
#if 0
 ...
 */
PJ_DEF(int)
pj_inet_aton(const pj_str_t *cp, struct pj_in_addr *inp)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_inet_pton(int af, const pj_str_t *src, void *dst)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_inet_ntop(int af, const void *src,
			 char *dst, int size)
...
pj_sock_socket(int af,
			   ...
			   pj_sock_t *sock)
{
	PJ_LOG(3, (THIS_FILE, "[sp][socket] create pj socket type:%d proto:%d", type, proto));
	PJ_CHECK_STACK();
	...
	*sock = socket(af, type, proto);
	...
	else
	{
		...
		{
			//TCP 类型 对一个对端已经关闭的 socket 调用两次 write，第二次 write 将会生成 SIGPIPE 信号，该信号默认结束进程。
			// 设置 socket 在进行写操作时不产生 SIGPIPE 信号。
			PJ_LOG(3, (THIS_FILE, "[sp][socket][tcp] make tcp socket donot signal pipe"));
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_bind_in(pj_sock_t sock,
				pj_uint32_t addr32,
				pj_uint16_t port)
{
	pj_sockaddr_in addr;
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_getpeername(pj_sock_t sock,
					pj_sockaddr_t *addr,
					int *namelen)
{
	PJ_CHECK_STACK();
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_getsockname(pj_sock_t sock,
					pj_sockaddr_t *addr,
					int *namelen)
{
	PJ_CHECK_STACK();
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_recvfrom(pj_sock_t sock,
				 void *buf,
				 pj_ssize_t *len,
				 unsigned flags,
				 pj_sockaddr_t *from,
...
/*
 * Get socket option.
 * https://blog.csdn.net/l_yangliu/article/details/7086256
 */
PJ_DEF(pj_status_t)
pj_sock_getsockopt(pj_sock_t sock,
				   pj_uint16_t level,
				   pj_uint16_t optname,
				   void *optval,
				   int *optlen)
...
/*
 * Set socket option.
 * level：选项定义的层次；目前仅支持SOL_SOCKET和IPPROTO_TCP层次。参数level 代表欲设置的网络层, 一般设成SOL_SOCKET 以存取socket 层
 * optname:
 * 		SO_DEBUG 打开或关闭排错模式
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_setsockopt(pj_sock_t sock,
				   pj_uint16_t level,
				   pj_uint16_t optname,
				   const void *optval,
				   int optlen)
{
	int status;
	PJ_CHECK_STACK();
	PJ_LOG(3, (THIS_FILE, "[socket]set socket %d option in level:%d name:%d value:%s", sock, level, optname, optval));
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_setsockopt_params(pj_sock_t sockfd,
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_connect(pj_sock_t sock,
				const pj_sockaddr_t *addr,
				int namelen)
{
	PJ_CHECK_STACK();
...
#if PJ_HAS_TCP
PJ_DEF(pj_status_t)
pj_sock_shutdown(pj_sock_t sock,
				 int how)
{
	PJ_CHECK_STACK();
 ...
 */
PJ_DEF(pj_status_t)
pj_sock_accept(pj_sock_t serverfd,
			   pj_sock_t *newsock,
			   pj_sockaddr_t *addr,
			   int *addrlen)
{
