 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_set_lock(pj_ioqueue_t *ioqueue,
                    pj_lock_t *lock,
 ...
 */
PJ_DEF(void *)
pj_ioqueue_get_user_data(pj_ioqueue_key_t *key)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_set_user_data(pj_ioqueue_key_t *key,
                         void *user_data,
                         void **old_data)
{
...
PJ_INLINE(int)
key_has_pending_write(pj_ioqueue_key_t *key)
{
    return !pj_list_empty(&key->write_list);
}

PJ_INLINE(int)
key_has_pending_read(pj_ioqueue_key_t *key)
{
    return !pj_list_empty(&key->read_list);
}

PJ_INLINE(int)
key_has_pending_accept(pj_ioqueue_key_t *key)
{
#if PJ_HAS_TCP
...
PJ_INLINE(int)
key_has_pending_connect(pj_ioqueue_key_t *key)
{
...
pj_bool_t ioqueue_dispatch_read_event(pj_ioqueue_t *ioqueue,
                                      pj_ioqueue_key_t *h)
{
    // PJ_LOG(3, (THIS_FILE, "[io][socket] ioqueue dispatch read event"));
    pj_status_t rc;
    ...
    if (!pj_list_empty(&h->accept_list))
    {
        ...
        // PJ_LOG(3, (THIS_FILE, "[io][socket][event] accept socket "));
	 ...
	 * hold the mutex while calling the callback) */
        ...
        {
            // PJ_LOG(3, (THIS_FILE, "[io][event] on accept complete"));
        ...
        if (key_has_pending_read(h))
    {
        ...
        if (read_op->op == PJ_IOQUEUE_OP_RECV_FROM)
        {
            ...
            // PJ_LOG(3, (THIS_FILE, "[io][socket][event] get data use recvfrom length:%d", bytes_read));
            //一般sendto()及recvfrom（）用于UDP，可用来接收、发送给指定的ip端
        ...
        {
            // PJ_LOG(3, (THIS_FILE, "[io][socket][event] get data use recv"));
            //一般send(),recv()用于TCP，面向连接的，无需再指定地址
            read_op->op = PJ_IOQUEUE_OP_NONE;
    ...
    defined(PJ_WIN32_WINCE) && PJ_WIN32_WINCE != 0
            ...
            // PJ_LOG(3, (THIS_FILE, "[io][socket][event] read from socket"));
...
#error "Implement read() for this platform!"
	 ...
	 * hold the mutex while calling the callback) */
        ...
        {
            // PJ_LOG(3, (THIS_FILE, "[io][socket][event] read complete"));
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_recv(pj_ioqueue_key_t *key,
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_recvfrom(pj_ioqueue_key_t *key,
                    pj_ioqueue_op_key_t *op_key,
                    void *buffer,
                    pj_ssize_t *length,
                    unsigned flags,
                    ...
                    int *addrlen)
{
    ...
    // PJ_LOG(3, (THIS_FILE, "[io][sp] flags is :%d", flags));
    /*
     * No data is immediately available.
     * Must schedule asynchronous operation to the ioqueue.
     */
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_send(pj_ioqueue_key_t *key,
                pj_ioqueue_op_key_t *op_key,
                const void *data,
                pj_ssize_t *length,
                unsigned flags)
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_sendto(pj_ioqueue_key_t *key,
                  pj_ioqueue_op_key_t *op_key,
                  const void *data,
                  pj_ssize_t *length,
                  pj_uint32_t flags,
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_accept(pj_ioqueue_key_t *key,
                  pj_ioqueue_op_key_t *op_key,
                  pj_sock_t *new_sock,
                  pj_sockaddr_t *local,
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_connect(pj_ioqueue_key_t *key,
                   const pj_sockaddr_t *addr,
                   int addrlen)
{
    pj_status_t status;
...
PJ_DEF(void)
pj_ioqueue_op_key_init(pj_ioqueue_op_key_t *op_key,
                       pj_size_t size)
{
 ...
 */
PJ_DEF(pj_bool_t)
pj_ioqueue_is_pending(pj_ioqueue_key_t *key,
                      pj_ioqueue_op_key_t *op_key)
{
 ...
 */
PJ_DEF(pj_status_t)
pj_ioqueue_post_completion(pj_ioqueue_key_t *key,
                           pj_ioqueue_op_key_t *op_key,
...
PJ_DEF(pj_status_t)
pj_ioqueue_set_default_concurrency(pj_ioqueue_t *ioqueue,
                                   pj_bool_t allow)
{
...
PJ_DEF(pj_status_t)
pj_ioqueue_set_concurrency(pj_ioqueue_key_t *key,
                           pj_bool_t allow)
{
    PJ_ASSERT_RETURN(key, PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pj_ioqueue_trylock_key(pj_ioqueue_key_t *key)
{
    if (key->grp_lock)
