 ...
 * 如果发生则将发生的socket fd和事件类型放入到events数组中。
...
{
    PJ_LOG(3, (THIS_FILE, "[io] create epoll ioqueue"));
   ...
   */
    PJ_LOG(4, ("pjlib", "[io]epoll I/O Queue created (%p)", ioqueue));
...
*/
    ...
    PJ_LOG(3, (THIS_FILE, "[io][socket] add socket %d to epoll %d", sock, ioqueue->epfd));
    ...
    if (status < 0)
    {
        ...
        key = NULL;
        TRACE_((THIS_FILE,
                "[io][epoll]pj_ioqueue_register_sock error: os_epoll_ctl rc=%d",
                status));
        goto on_return;
    }
...
pj_ioqueue_poll(pj_ioqueue_t *ioqueue, const pj_time_val *timeout)
{
    ...
    TRACE_((THIS_FILE, "[io][epoll]start os_epoll_wait, msec=%d", msec));
    pj_get_timestamp(&t1);
...
#endif
        TRACE_((THIS_FILE, "[io][epoll]os_epoll_wait timed out"));
        return count;
    }
    else if (count < 0)
    {
        TRACE_((THIS_FILE, "[io][epoll]os_epoll_wait error"));
        return -pj_get_netos_error();
    }

    pj_get_timestamp(&t2);
    TRACE_((THIS_FILE, "[io][epoll]os_epoll_wait returns %d, time=%d usec",
    ...
    for (event_cnt = 0, i = 0; i < count; ++i)
    {
        ...
        TRACE_((THIS_FILE, "[io][epoll]event %d: events=%d", i, events[i].events));
    ...
    for (i = 0; i < event_cnt; ++i)
    {
        ...
        if (processed_cnt < PJ_IOQUEUE_MAX_EVENTS_IN_SINGLE_POLL)
        {
            ...
            {
            case READABLE_EVENT:
                PJ_LOG(3, (THIS_FILE,"[io][epoll] receive read evet dipatch it"));
                if (ioqueue_dispatch_read_event(ioqueue, queue[i].key))
                    ++processed_cnt;
                break;
            case WRITEABLE_EVENT:
                PJ_LOG(3, (THIS_FILE,"[io][epoll] receive write evet dipatch it"));
                if (ioqueue_dispatch_write_event(ioqueue, queue[i].key))
                    ++processed_cnt;
                break;
            case EXCEPTION_EVENT:
                PJ_LOG(3, (THIS_FILE,"[io][epoll] receive exception evet dipatch it"));
