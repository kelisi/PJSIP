													...
													pj_qos_params *param)
{
	...
	PJ_LOG(3, (THIS_FILE, "[qos] val 1 is :%d", val));
	...
	PJ_LOG(3, (THIS_FILE, "[qos] val2 is :%d", val));
...
PJ_DEF(pj_status_t)
pj_sock_set_qos_params(pj_sock_t sock,
					   pj_qos_params *param)
{
	PJ_LOG(3, ("sock_qos", "[qos] set qos param to sock:%d", sock));
	PJ_LOG(3, ("sock_qos", "[qos] set qos param->flags:%d ->dscp_val:%d", param->flags, param->dscp_val));
	pj_status_t status;
...
PJ_DEF(pj_status_t)
pj_sock_set_qos_type(pj_sock_t sock,
					 pj_qos_type type)
{
	pj_status_t status;
	pj_qos_params param;
...
PJ_DEF(pj_status_t)
pj_sock_get_qos_params(pj_sock_t sock,
					   pj_qos_params *p_param)
{
	pj_status_t status;
	int val, optlen;
...
PJ_DEF(pj_status_t)
pj_sock_get_qos_type(pj_sock_t sock,
					 pj_qos_type *p_type)
{
	pj_qos_params param;
	pj_status_t status;
