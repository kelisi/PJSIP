...
{
    PJ_LOG(3, (THIS_FILE, "[core][timer] create timer heap with max size:%ld", size));
    pj_timer_heap_t *ht;
    pj_size_t i;
                   ...
                   pj_time_val *next_delay)
{
    ...
    //获取二叉堆顶元素，判断是否已经到达超时时间，若超时后触发回调，并继续判断下一个直到数目超过最大限制或未达到超时时间
    while (ht->cur_size &&
