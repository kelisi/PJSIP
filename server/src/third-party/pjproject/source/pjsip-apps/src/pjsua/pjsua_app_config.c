...
static void usage(void)
{
	...
	puts("  --quality=N         Specify media quality (0-10, default=" xstr(PJSUA_DEFAULT_CODEC_QUALITY) ")");
	...
	puts("  --ec-tail=MSEC      Set echo canceller tail length (default=" xstr(PJSUA_DEFAULT_EC_TAIL_LEN) ")");
	puts("  --ec-opt=OPT        Select echo canceller algorithm (0=default, ");
	puts("                        1=speex, 2=suppressor, 3=WebRtc)");
	puts("  --ilbc-mode=MODE    Set iLBC codec mode (20 or 30, default is " xstr(PJSUA_DEFAULT_ILBC_MODE) ")");
	puts("  --capture-dev=id    Audio capture device ID (default=-1)");
	puts("  --playback-dev=id   Audio playback device ID (default=-1)");
	puts("  --capture-lat=N     Audio capture latency, in ms (default=" xstr(PJMEDIA_SND_DEFAULT_REC_LATENCY) ")");
	puts("  --playback-lat=N    Audio playback latency, in ms (default=" xstr(PJMEDIA_SND_DEFAULT_PLAY_LATENCY) ")");
...
{
	PJ_LOG(3, (THIS_FILE, "[app][cfg] read config file :%s", filename));
	int i;
	FILE *fhnd;
	char line[200];
	int argc = 0;
...
{
	PJ_LOG(3, (THIS_FILE, "[app][cfg] parse args"));
	int c;
	int option_index;
	...
	while ((c = pj_getopt_long(argc, argv, "", long_options, &option_index)) != -1)
	{
		pj_str_t tmp;
		long lval;
		PJ_LOG(3, (THIS_FILE, "[app][cfg] read config:%d", c));
		switch (c)
		{
...
#endif
		case OPT_QOS:
			PJ_LOG(3, (THIS_FILE, "[app][cfg][init] enable qos"));
...
pj_status_t load_config(int argc,
						...
						pj_str_t *uri_arg)
{
	PJ_LOG(3, (THIS_FILE, "[app][cfg] load config %s", uri_arg->ptr));
	pj_status_t status;
