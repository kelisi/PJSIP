...
#if PJMEDIA_HAS_FFMPEG_CODEC_H264
        {
            {PJMEDIA_FORMAT_H264, PJMEDIA_RTP_PT_H264, {"H264", 4}, {"Constrained Baseline (level=30, pack=1)", 39}},
            0,
            {720, 480},
            {15, 1},
            256000,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_ffmpeg_vid_init(pjmedia_vid_codec_mgr *mgr,
                              pj_pool_factory *pf)
{
    PJ_LOG(3, (THIS_FILE, "[video][codec][ffmpeg] init & register codec factory to pjmedia endpoint"));
    pj_pool_t *pool;
    AVCodec *c;
    pj_status_t status;
    unsigned i;
...
#define AVMEDIA_TYPE_VIDEO CODEC_TYPE_VIDEO
	 ...
	 * supported fps) are in the encoder.
        ...
        PJ_LOG(3, (THIS_FILE, "[video][ffmpeg]enum codec:[%s] id:[%d]", c->name, c->id));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_ffmpeg_vid_deinit(void)
{
...
static pj_status_t ffmpeg_codec_init(pjmedia_vid_codec *codec,
                                     pj_pool_t *pool)
{
    PJ_LOG(3, (THIS_FILE, "[video][ffmpeg] codec init"));
    PJ_UNUSED_ARG(codec);
    PJ_UNUSED_ARG(pool);
    return PJ_SUCCESS;
}
                                     ...
                                     pj_mutex_t *ff_mutex)
{
    ...
    PJ_LOG(3, (THIS_FILE, "[video][ffmpeg] init encoder param:\
                                            pix_fmt:%d \n\
                                            width:[%d] height[%d] \n\
                                            fps:[%d/%d] \n \
                                            bitrate:%d",
...
static pj_status_t ffmpeg_codec_open(pjmedia_vid_codec *codec,
                                     pjmedia_vid_codec_param *attr)
{
    PJ_LOG(3, (THIS_FILE, "[video][ffmpeg] codec open"));
    ffmpeg_private *ff;
    pj_status_t status;
    pj_mutex_t *ff_mutex;
