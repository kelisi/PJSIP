 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_opus_init(pjmedia_endpt *endpt)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_codec_opus_deinit(void)
{
    pj_status_t status;
    pjmedia_codec_mgr *codec_mgr;
                              ...
                              pjmedia_codec_param *attr)
{
    ...
    /* Set bitrate */
    opus_encoder_ctl(opus_data->enc, OPUS_SET_BITRATE(auto_bit_rate ? OPUS_AUTO : attr->info.avg_bps));
    /* Set VAD */
    ...
    opus_data->dec_frame[0].buf = pj_pool_zalloc(opus_data->pool,
                                                 (opus_data->cfg.sample_rate / 1000) * 60 * attr->info.channel_cnt * 2 /* bytes per sample */);
                                ...
                                const pjmedia_codec_param *attr)
{
    ...
    opus_encoder_ctl(opus_data->enc, OPUS_SET_BITRATE(attr->info.avg_bps ? attr->info.avg_bps : OPUS_AUTO));
    /* Set VAD */
    opus_encoder_ctl(opus_data->enc, OPUS_SET_DTX(attr->setting.vad ? 1 : 0));
    /* Set PLC */
    opus_encoder_ctl(opus_data->enc,
