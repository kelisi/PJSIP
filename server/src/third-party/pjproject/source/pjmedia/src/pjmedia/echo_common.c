 ...
 */
PJ_DEF(pj_status_t)
pjmedia_echo_create(pj_pool_t *pool,
                    unsigned clock_rate,
                    unsigned samples_per_frame,
                    unsigned tail_ms,
                    unsigned latency_ms,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_echo_create2(pj_pool_t *pool,
                     unsigned clock_rate,
                     unsigned channel_count,
                     unsigned samples_per_frame,
                     unsigned tail_ms,
                     ...
                     pjmedia_echo_state **p_echo)
{
             ...
             (options & PJMEDIA_ECHO_ALGO_MASK) == PJMEDIA_ECHO_DEFAULT)
    {
        ec->op = &ipp_aec_op;
             ...
             (options & PJMEDIA_ECHO_ALGO_MASK) == PJMEDIA_ECHO_DEFAULT)
    {
        PJ_LOG(3, (THIS_FILE, "[audio] use webRTC AEC"));
        ec->op = &webrtc_aec_op;
#endif
    }
    else
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_echo_destroy(pjmedia_echo_state *echo)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_echo_playback(pjmedia_echo_state *echo,
                      pj_int16_t *play_frm)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_echo_capture(pjmedia_echo_state *echo,
                     pj_int16_t *rec_frm,
                     unsigned options)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_echo_cancel(pjmedia_echo_state *echo,
                    pj_int16_t *rec_frm,
                    const pj_int16_t *play_frm,
                    unsigned options,
                    void *reserved)
