 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_udp_create(pjmedia_endpt *endpt,
							 const char *name,
							 int port,
							 unsigned options,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_udp_create2(pjmedia_endpt *endpt,
							  const char *name,
							  const pj_str_t *addr,
							  int port,
							  unsigned options,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_udp_create3(pjmedia_endpt *endpt,
							  int af,
							  const char *name,
							  const pj_str_t *addr,
							  int port,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_transport_udp_attach(pjmedia_endpt *endpt,
							 const char *name,
	...
	(PJMEDIA_TRANSPORT_SWITCH_REMOTE_ADDR == 1)
			...
			(udp->options & PJMEDIA_UDP_NO_SRC_ADDR_CHECKING) == 0)
		{
	         ...
	         */
				...
				pj_sockaddr_cp(&udp->rem_rtcp_addr, &udp->rem_rtp_addr);
				port = (pj_uint16_t)(pj_sockaddr_get_port(&udp->rem_rtp_addr) + 1);
									  ...
									  pjmedia_transport_info *info)
{
	...
	info->sock_info.rtcp_addr_name = (udp->use_rtcp_mux ? udp->rtp_addr_name : udp->rtcp_addr_name);
...
/**
 * @brief 将 RTP Socket相关信息绑定到transport_udp *)tp上
 * 
 * @param tp 
 * @param user_data 
 * @param rem_addr 
										...
										pj_size_t size)
{
	...
	sent = size;
	status = pj_ioqueue_sendto((udp->use_rtcp_mux ? udp->rtp_key : udp->rtcp_key), &udp->rtcp_write_op,
