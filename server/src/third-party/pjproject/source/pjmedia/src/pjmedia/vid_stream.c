...
PJ_INLINE(int)
trace_jb_print_state(pjmedia_vid_stream *stream,
	...
	if (param->src_addr)
	{
		...
		else
		{
		     ...
		     */
				...
				if (stream->info.has_rem_ssrc && !seq_st.status.flag.badssrc && stream->rem_rtp_flag != 1)
				{
							 ...
							 pjmedia_frame *frame)
{
	...
	if ((stream->num_keyframe) &&
		((frame_out.bit_info & PJMEDIA_VID_FRM_KEYFRAME) == PJMEDIA_VID_FRM_KEYFRAME))
	{
							 ...
							 pjmedia_frame *frame)
{
	...
	if (stream->fmt_event.type != PJMEDIA_EVENT_NONE)
	{
		...
		dump_port_info(fmt_chg_data->dir == PJMEDIA_DIR_DECODING ? stream->dec : stream->enc,
					   "changed");
	...
	pjmedia_vid_stream **p_stream)
{
	...
	PJ_LOG(3, (THIS_FILE, "[video] video stream direction:%d \n\
												rtcp interval:%d \n\
												enc mtu:%d \n\
												keyframe num:%d \n\
												rtp cname:%s \n",
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_stream_destroy(pjmedia_vid_stream *stream)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_stream_get_port(pjmedia_vid_stream *stream,
							pjmedia_dir dir,
							pjmedia_port **p_port)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_stream_reset_stat(pjmedia_vid_stream *stream)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_stream_start(pjmedia_vid_stream *stream)
{
 ...
 */
PJ_DEF(pj_bool_t)
pjmedia_vid_stream_is_running(pjmedia_vid_stream *stream,
							  pjmedia_dir dir)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_stream_pause(pjmedia_vid_stream *stream,
						 pjmedia_dir dir)
{
	PJ_ASSERT_RETURN(stream, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_vid_stream_resume(pjmedia_vid_stream *stream,
						  pjmedia_dir dir)
{
	PJ_ASSERT_RETURN(stream, PJ_EINVAL);
