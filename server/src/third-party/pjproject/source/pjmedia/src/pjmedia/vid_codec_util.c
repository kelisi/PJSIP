...
/* ITU(国际电信联盟) resolution definition */
...
/* H263 fmtp parser */
PJ_DEF(pj_status_t)
pjmedia_vid_codec_parse_h263_fmtp(
	     ...
	     */
			for (i = 0; i < fmtp_rem.mpi_cnt && !matched; ++i)
			{
				...
				for (j = 0; j < fmtp_loc.mpi_cnt; ++j)
				{
					...
					if (diff < min_diff)
					{
						size = rem_sq < loc_sq ? fmtp_rem.mpi[i].size : fmtp_loc.mpi[j].size;
						mpi = PJ_MAX(fmtp_rem.mpi[i].val,
									 fmtp_loc.mpi[j].val);
					}
				}
...
PJ_DEF(pj_status_t)
pjmedia_vid_codec_h264_match_sdp(pj_pool_t *pool,
								 ...
								 unsigned option)
{
	// PJ_LOG(3, (THIS_FILE, "[video][codec][h264] match with sdp"));
	const pj_str_t PROFILE_LEVEL_ID = {"profile-level-id", 16};
	const pj_str_t PACKETIZATION_MODE = {"packetization-mode", 18};
