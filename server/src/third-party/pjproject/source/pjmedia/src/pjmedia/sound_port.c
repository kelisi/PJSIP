...
PJ_DEF(void)
pjmedia_snd_port_param_default(pjmedia_snd_port_param *prm)
{
                                      ...
                                      pjmedia_snd_port *snd_port)
{
    ...
    if (param_copy.flags & PJMEDIA_AUD_DEV_CAP_EC)
    {
            ...
            (snd_port->aud_caps & PJMEDIA_AUD_DEV_CAP_EC))
        {
            /* Device supports EC */
            /* Nothing to do */
            PJ_LOG(3, (THIS_FILE, "[sound][port] device support ec  do nothing "));
        }
        else
        {
	     ...
	     * device parameters
	     */
            PJ_LOG(3, (THIS_FILE, "[sound][port] device not support ec use software ec "));
    ...
    {
        PJ_LOG(3, (THIS_FILE, "[sound][stream] start audio stream"));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_create(pj_pool_t *pool,
                        int rec_id,
                        int play_id,
                        unsigned clock_rate,
                        unsigned channel_count,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_create_rec(pj_pool_t *pool,
                            int dev_id,
                            unsigned clock_rate,
                            unsigned channel_count,
                            unsigned samples_per_frame,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_create_player(pj_pool_t *pool,
                               int dev_id,
                               unsigned clock_rate,
                               unsigned channel_count,
                               unsigned samples_per_frame,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_create2(pj_pool_t *pool,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_destroy(pjmedia_snd_port *snd_port)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_set_ec(pjmedia_snd_port *snd_port,
                        pj_pool_t *pool,
                        unsigned tail_ms,
                        unsigned options)
{
...
/* Get AEC tail length */
PJ_DEF(pj_status_t)
pjmedia_snd_port_get_ec_tail(pjmedia_snd_port *snd_port,
                             unsigned *p_length)
{
...
pjmedia_snd_port_get_clock_src(pjmedia_snd_port *snd_port,
                               pjmedia_dir dir)
{
    return (dir == PJMEDIA_DIR_CAPTURE ? &snd_port->cap_clocksrc : &snd_port->play_clocksrc);
}
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_connect(pjmedia_snd_port *snd_port,
                         pjmedia_port *port)
{
 ...
 */
PJ_DEF(pjmedia_port *)
pjmedia_snd_port_get_port(pjmedia_snd_port *snd_port)
{
    PJ_ASSERT_RETURN(snd_port, NULL);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_snd_port_disconnect(pjmedia_snd_port *snd_port)
{
    PJ_ASSERT_RETURN(snd_port, PJ_EINVAL);
