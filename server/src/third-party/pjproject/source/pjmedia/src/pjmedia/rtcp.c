...
#define JAN_1970 (2208988800UL)
	...
	ntp->hi = (pj_uint32_t)((ts.u64 - sess->ts_base.u64) / sess->ts_freq.u64) + sess->tv_base.sec + JAN_1970;
 ...
 */
PJ_DEF(void)
pjmedia_rtcp_init(pjmedia_rtcp_session *sess,
				  char *name,
				  unsigned clock_rate,
...
PJ_DEF(void)
pjmedia_rtcp_fini(pjmedia_rtcp_session *sess)
{
...
PJ_DEF(void)
pjmedia_rtcp_rx_rtp(pjmedia_rtcp_session *sess,
					unsigned seq,
					unsigned rtp_ts,
					unsigned payload)
{
...
PJ_DEF(void)
pjmedia_rtcp_rx_rtp2(pjmedia_rtcp_session *sess,
					 unsigned seq,
					 unsigned rtp_ts,
					 unsigned payload,
					 pj_bool_t discarded)
...
PJ_DEF(void)
pjmedia_rtcp_tx_rtp(pjmedia_rtcp_session *sess,
...
PJ_DEF(void)
pjmedia_rtcp_build_rtcp(pjmedia_rtcp_session *sess,
...
PJ_DEF(pj_status_t)
pjmedia_rtcp_build_rtcp_bye(pjmedia_rtcp_session *session,
							void *buf,
							pj_size_t *length,
...
PJ_DEF(pj_status_t)
pjmedia_rtcp_enable_xr(pjmedia_rtcp_session *sess,
					   pj_bool_t enable)
{
#if defined(PJMEDIA_HAS_RTCP_XR) && (PJMEDIA_HAS_RTCP_XR != 0)
