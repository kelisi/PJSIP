...
PJ_DEF(void)
pjmedia_vid_port_param_default(pjmedia_vid_port_param *prm)
{
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_create(pj_pool_t *pool,
                        ...
                        pjmedia_vid_port **p_vid_port)
{
	 ...
	 */
        ...
        status = pjmedia_clock_create2(pool, &param,
                                       ...
                                       (vp->dir & PJMEDIA_DIR_ENCODING) ? &enc_clock_cb : &dec_clock_cb,
                                       vp, &vp->clock);
        if (status != PJ_SUCCESS)
            goto on_error;
    }
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_connect(pjmedia_vid_port *vp,
                         pjmedia_port *port,
                         pj_bool_t destroy)
{
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_disconnect(pjmedia_vid_port *vp)
{
    PJ_ASSERT_RETURN(vp && vp->role == ROLE_ACTIVE, PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pjmedia_vid_port_start(pjmedia_vid_port *vp)
{
    pj_status_t status;
...
PJ_DEF(pj_bool_t)
pjmedia_vid_port_is_running(pjmedia_vid_port *vp)
{
    return pjmedia_vid_dev_stream_is_running(vp->strm);
}

PJ_DEF(pj_status_t)
pjmedia_vid_port_stop(pjmedia_vid_port *vp)
{
    pj_status_t status;
...
PJ_DEF(void)
pjmedia_vid_port_destroy(pjmedia_vid_port *vp)
{
