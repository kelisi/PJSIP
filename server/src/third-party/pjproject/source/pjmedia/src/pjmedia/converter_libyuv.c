...
typedef int (*scale_planar_method)(const uint8 *src_y, int src_stride_y,
...
#define GET_MAP(src)              \
	...
	act[act_idx].src_fmt_info.apply_param.size = (!act_idx) ? *src_size : act[act_idx - 1].dst_fmt_info.apply_param.size;

	/* Destination buffer size is not the same as source buffer size 
       when scaling. */
	act[act_idx].dst_fmt_info.apply_param.size =
		(METHOD_IS_SCALE(act[act_idx].act_type)) ? *dst_size : act[act_idx].src_fmt_info.apply_param.size;
									   ...
									   pjmedia_frame *dst_frame)
{
	...
	for (; i < lconv->act_num; ++i)
	{
		...
		struct fmt_info *src_fmt_info = (i == 0) ? &lconv->act[i].src_fmt_info : &lconv->act[i - 1].dst_fmt_info;
