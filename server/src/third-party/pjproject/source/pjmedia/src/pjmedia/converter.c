...
PJ_DEF(pj_status_t)
pjmedia_converter_mgr_create(pj_pool_t *pool,
...
#if defined(PJMEDIA_HAS_VIDEO) && (PJMEDIA_HAS_VIDEO != 0) && \
    defined(PJMEDIA_HAS_LIBSWSCALE) && (PJMEDIA_HAS_LIBSWSCALE != 0)
    /*libswscale实现了各种像素图像格式转换，如YUV转RGB、图像大小缩放等，效率高*/
...
PJ_DEF(pjmedia_converter_mgr *)
pjmedia_converter_mgr_instance(void)
{
...
PJ_DEF(pj_status_t)
pjmedia_converter_create(pjmedia_converter_mgr *mgr,
                         pj_pool_t *pool,
...
PJ_DEF(pj_status_t)
pjmedia_converter_convert(pjmedia_converter *cv,
