 ...
 */
PJ_DEF(const char *)
pjmedia_sdp_neg_state_str(pjmedia_sdp_neg_state state)
{
									  ...
									  pjmedia_sdp_neg **p_neg)
{
	...
	*p_neg = NULL;
	...
	if (initial)
	{
		PJ_LOG(3, (THIS_FILE, "[sp][call][sdp] local sdp not null"));
	...
	{
		PJ_LOG(3, (THIS_FILE, "[sp][call][sdp] local sdp null"));
...
PJ_DEF(pj_bool_t)
pjmedia_sdp_neg_has_local_answer(pjmedia_sdp_neg *neg)
{
...
{
	PJ_LOG(3, (THIS_FILE, "[call] process_answer"));
 ...
 */
PJ_INLINE(void)
rewrite_pt(pj_pool_t *pool, pj_str_t *attr_val,
	     ...
	     */
			...
			for (j = 0; j < slave->desc.fmt_count; ++j)
			{
				if (!pj_strcmp(&master->desc.fmt[i], &slave->desc.fmt[j]))
				{
					...
					pt_answer[pt_answer_count++] = prefer_remote_codec_order ? preanswer->desc.fmt[j] : preanswer->desc.fmt[i];
					break;
				}
			}
		}
...
{
	PJ_LOG(3, (THIS_FILE, "[call] create_answer"));
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_negotiate(pj_pool_t *pool,
						  pjmedia_sdp_neg *neg,
						  pj_bool_t allow_asym)
{
	PJ_LOG(3, (THIS_FILE, "[call][sdp] negotiate SDP"));
	pj_status_t status;
...
PJ_DEF(pj_status_t)
pjmedia_sdp_neg_fmt_match(pj_pool_t *pool,
						  pjmedia_sdp_media *offer,
						  unsigned o_fmt_idx,
						  pjmedia_sdp_media *answer,
						  unsigned a_fmt_idx,
