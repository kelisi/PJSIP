...
PJ_DEF(pj_status_t)
pjmedia_clock_src_init(pjmedia_clock_src *clocksrc,
...
pjmedia_clock_src_get_time_msec(const pjmedia_clock_src *clocksrc)
{
    ...
    return (pj_uint32_t)((1.0 * ts.u32.hi * 0xFFFFFFFFUL + ts.u32.lo) * 1000.0 / clocksrc->clock_rate);
#else
    if (ts.u32.lo > 0x3FFFFFUL)
        return (pj_uint32_t)(0xFFFFFFFFUL / clocksrc->clock_rate * ts.u32.hi * 1000UL + ts.u32.lo / clocksrc->clock_rate *
                                                                                            1000UL);
    else
        return (pj_uint32_t)(0xFFFFFFFFUL / clocksrc->clock_rate * ts.u32.hi * 1000UL + ts.u32.lo * 1000UL /
                                                                                            clocksrc->clock_rate);
#endif
}
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_clock_create(pj_pool_t *pool,
                     unsigned clock_rate,
...
PJ_DEF(pj_status_t)
pjmedia_clock_create2(pj_pool_t *pool,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_clock_start(pjmedia_clock *clock)
{
    pj_timestamp now;
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_clock_stop(pjmedia_clock *clock)
{
    PJ_ASSERT_RETURN(clock != NULL, PJ_EINVAL);
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_clock_modify(pjmedia_clock *clock,
                     const pjmedia_clock_param *param)
{
    clock->interval.u64 = param->usec_interval * clock->freq.u64 /
                          USEC_IN_SEC;
...
/* Calculate next tick */
PJ_INLINE(void)
clock_calc_next_tick(pjmedia_clock *clock,
                     pj_timestamp *now)
{
 ...
 */
PJ_DEF(pj_bool_t)
pjmedia_clock_wait(pjmedia_clock *clock,
                   pj_bool_t wait,
                   pj_timestamp *ts)
{
    pj_timestamp now;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_clock_destroy(pjmedia_clock *clock)
{
    PJ_ASSERT_RETURN(clock != NULL, PJ_EINVAL);
