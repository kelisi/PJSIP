...
PJ_DEF(pjmedia_aud_subsys *)
pjmedia_get_aud_subsys(void)
{
    return &aud_subsys;
}
...
/* API: init driver */
PJ_DEF(pj_status_t)
pjmedia_aud_driver_init(unsigned drv_idx,
                        pj_bool_t refresh)
{
...
/* API: deinit driver */
PJ_DEF(void)
pjmedia_aud_driver_deinit(unsigned drv_idx)
{
    pjmedia_aud_driver *drv = &aud_subsys.drv[drv_idx];
...
PJ_DEF(const char *)
pjmedia_aud_dev_cap_name(pjmedia_aud_dev_cap cap,
                         const char **p_desc)
{
    const char *desc;
    unsigned i;
...
PJ_DEF(pj_status_t)
pjmedia_aud_param_set_cap(pjmedia_aud_param *param,
                          pjmedia_aud_dev_cap cap,
                          const void *pval)
{
    void *cap_ptr;
...
PJ_DEF(pj_status_t)
pjmedia_aud_dev_get_info(pjmedia_aud_dev_index id,
                         pjmedia_aud_dev_info *info)
{
    pjmedia_aud_dev_factory *f;
    unsigned index;
...
/* API: find device */
PJ_DEF(pj_status_t)
pjmedia_aud_dev_lookup(const char *drv_name,
                       const char *dev_name,
                       pjmedia_aud_dev_index *id)
{
...
PJ_DEF(pj_status_t)
pjmedia_aud_stream_get_cap(pjmedia_aud_stream *strm,
                           pjmedia_aud_dev_cap cap,
                           void *value)
{
...
PJ_DEF(pj_status_t)
pjmedia_aud_stream_set_cap(pjmedia_aud_stream *strm,
                           pjmedia_aud_dev_cap cap,
                           const void *value)
{
...
{
    PJ_LOG(3, (THIS_FILE,"[sound][stream] start sound [pjmedia_aud_stream] stream;"));
