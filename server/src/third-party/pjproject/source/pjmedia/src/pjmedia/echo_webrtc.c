...
#else
    PJ_LOG(3, (THIS_FILE, "[aec][webrtc] set platform config"));
 ...
 */
PJ_DEF(pj_status_t)
webrtc_aec_create(pj_pool_t *pool,
                  unsigned clock_rate,
                  unsigned channel_count,
                  unsigned samples_per_frame,
                  unsigned tail_ms,
                  ...
                  void **p_echo)
{
    PJ_LOG(3, (THIS_FILE, "[aec][webrtc] create webrtc_ec"));
    webrtc_ec *echo;
    int status;
 ...
 */
PJ_DEF(pj_status_t)
webrtc_aec_cancel_echo(void *state,
                       pj_int16_t *rec_frm,
