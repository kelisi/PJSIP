 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_create(pj_pool_t *pool,
					unsigned max_ports,
					unsigned clock_rate,
					...
					pjmedia_conf **p_conf)
{
	...
	PJ_LOG(5, (THIS_FILE, "[media][sound][conference]Creating conference bridge with %d ports",
			   max_ports));
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_destroy(pjmedia_conf *conf)
{
	unsigned i, ci;
 ...
 */
PJ_DEF(pjmedia_port *)
pjmedia_conf_get_master_port(pjmedia_conf *conf)
{
	/* Sanity check. */
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_set_port0_name(pjmedia_conf *conf,
							const pj_str_t *name)
{
	pj_size_t len;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_add_port(pjmedia_conf *conf,
					  pj_pool_t *pool,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_add_passive_port(pjmedia_conf *conf,
							  pj_pool_t *pool,
							  const pj_str_t *name,
							  unsigned clock_rate,
							  unsigned channel_count,
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_configure_port(pjmedia_conf *conf,
							unsigned slot,
							pjmedia_port_op tx,
							pjmedia_port_op rx)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_connect_port(pjmedia_conf *conf,
						  unsigned src_slot,
						  unsigned sink_slot,
						  int adj_level)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_disconnect_port(pjmedia_conf *conf,
							 unsigned src_slot,
							 unsigned sink_slot)
{
	struct conf_port *src_port, *dst_port;
 ...
 */
PJ_DEF(unsigned)
pjmedia_conf_get_connect_count(pjmedia_conf *conf)
{
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_remove_port(pjmedia_conf *conf,
						 unsigned port)
{
	struct conf_port *conf_port;
	unsigned i;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_enum_ports(pjmedia_conf *conf,
						unsigned ports[],
						unsigned *p_count)
{
	unsigned i, count = 0;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_get_port_info(pjmedia_conf *conf,
						   unsigned slot,
...
PJ_DEF(pj_status_t)
pjmedia_conf_get_ports_info(pjmedia_conf *conf,
							unsigned *size,
							pjmedia_conf_port_info info[])
{
	unsigned i, count = 0;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_adjust_rx_level(pjmedia_conf *conf,
							 unsigned slot,
							 int adj_level)
{
	struct conf_port *conf_port;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_adjust_tx_level(pjmedia_conf *conf,
							 unsigned slot,
							 int adj_level)
{
	struct conf_port *conf_port;
 ...
 */
PJ_DEF(pj_status_t)
pjmedia_conf_adjust_conn_level(pjmedia_conf *conf,
							   unsigned src_slot,
							   unsigned sink_slot,
							   int adj_level)
{
...
static pj_status_t get_frame(pjmedia_port *this_port,
							 pjmedia_frame *frame)
{
	// TRACE_((THIS_FILE, "[media] get frame"));
	pjmedia_conf *conf = (pjmedia_conf *)this_port->port_data.pdata;
