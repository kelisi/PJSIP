...
PJ_DEF(void)
pj_cis_buf_init(pj_cis_buf_t *cis_buf)
{
...
PJ_DEF(pj_status_t)
pj_cis_init(pj_cis_buf_t *cis_buf, pj_cis_t *cis)
{
    unsigned i;
    // cis_buf[256]
    ...
    for (i = 0; i < PJ_CIS_MAX_INDEX; ++i)
    {
        ...
        {
            //pconst.pjsip_DIGIT_SPEC->cis_id = 0
            cis->cis_id = i;
            //cis_buf->use_mask = 0x00000001
...
PJ_DEF(pj_status_t)
pj_cis_dup(pj_cis_t *new_cis, pj_cis_t *existing)
{
    pj_status_t status;
    unsigned i;
