...
PJ_DEF(pj_str_t)
pj_str_unescape(pj_pool_t *pool, const pj_str_t *src_str)
{
...
PJ_DEF(pj_str_t *)
pj_strcpy_unescape(pj_str_t *dst_str,
...
PJ_DEF(pj_ssize_t)
pj_strncpy2_escape(char *dst_str, const pj_str_t *src_str,
...
PJ_DEF(pj_str_t *)
pj_strncpy_escape(pj_str_t *dst_str,
				  const pj_str_t *src_str,
				  pj_ssize_t max, const pj_cis_t *unres)
{
...
pj_itoa(int num, char *ret, int radix)
{
	...
	do
	{
		ret[i++] = index[unum % (unsigned)radix];
	} while ((unum /= radix) != 0);
