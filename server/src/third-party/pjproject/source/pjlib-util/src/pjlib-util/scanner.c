...
PJ_DEF(void)
pj_cis_add_range(pj_cis_t *cis, int cstart, int cend)
{
    /* Can not set zero. This is the requirement of the parser. */
    pj_assert(cstart > 0);

    // PJ_LOG(3, (THIS_FILE, "[sp][parse][cis] range from :%d to:%d cis_id:%d", cstart, cend, cis->cis_id));
    while (cstart != cend)
    {
        //TODO:将cis_buf[256]中的start-end
        PJ_CIS_SET(cis, cstart);
        // PJ_LOG(3, (THIS_FILE, "[sp][parse][cis] status cis_buf[%d]=%d", cstart, cis->cis_buf[cstart]));

        ++cstart;
    }

    // PJ_LOG(3, (THIS_FILE, "[sp][parse][cis] status is:%d", cis->cis_buf));
}

PJ_DEF(void)
pj_cis_add_alpha(pj_cis_t *cis)
{
...
PJ_DEF(void)
pj_cis_add_str(pj_cis_t *cis, const char *str)
{
    // PJ_LOG(3, (THIS_FILE, "[sp][parse] pj_cis_add_str:[  %s  ]", str));

    while (*str)
    {
        PJ_CIS_SET(cis, *str);
        // PJ_LOG(3, (THIS_FILE, "[sp][parse] cis add ascii after cis_buf[%d]=%d", *str, cis->cis_buf[*str]));
...
PJ_DEF(void)
pj_cis_add_cis(pj_cis_t *cis, const pj_cis_t *rhs)
{
    int i;
    for (i = 0; i < 256; ++i)
    {
...
PJ_DEF(void)
pj_cis_del_range(pj_cis_t *cis, int cstart, int cend)
{
    while (cstart != cend)
    {
...
PJ_DEF(void)
pj_cis_del_str(pj_cis_t *cis, const char *str)
{
    while (*str)
    {
        PJ_CIS_CLR(cis, *str);
...
PJ_DEF(void)
pj_cis_invert(pj_cis_t *cis)
{
    unsigned i;
    /* Can not set zero. This is the requirement of the parser. */
    for (i = 1; i < 256; ++i)
    {
        //如果cis_buf[i]中，1<<cis_id所在的位为1，则将该位反转置为0，否则置为1
        if (PJ_CIS_ISSET(cis, i))
            PJ_CIS_CLR(cis, i);
        else
            PJ_CIS_SET(cis, i);

        // PJ_LOG(3, (THIS_FILE, "[sp][parse] pj_cis_invert invert cis_buf[%d]=%d", i, cis->cis_buf[i]));
    }
}

PJ_DEF(void)
pj_scan_init(pj_scanner *scanner, char *bufstart,
...
PJ_DEF(void)
pj_scan_fini(pj_scanner *scanner)
{
    PJ_CHECK_STACK();
...
PJ_DEF(void)
pj_scan_skip_whitespace(pj_scanner *scanner)
{
    //这个关键字请求编译器尽可能的将变量存在CPU 内部寄存器中而不是通过内存寻址访问以提高效率
...
PJ_DEF(void)
pj_scan_skip_line(pj_scanner *scanner)
{
...
PJ_DEF(int)
pj_scan_peek(pj_scanner *scanner,
...
PJ_DEF(int)
pj_scan_peek_n(pj_scanner *scanner,
...
PJ_DEF(int)
pj_scan_peek_until(pj_scanner *scanner,
                   const pj_cis_t *spec,
                   pj_str_t *out)
{
    register char *s = scanner->curptr;
...
PJ_DEF(void)
pj_scan_get(pj_scanner *scanner,
            const pj_cis_t *spec, pj_str_t *out)
{
    register char *s = scanner->curptr;
...
PJ_DEF(void)
pj_scan_get_unescape(pj_scanner *scanner,
                     const pj_cis_t *spec, pj_str_t *out)
{
    register char *s = scanner->curptr;
    char *dst = s;
...
PJ_DEF(void)
pj_scan_get_quote(pj_scanner *scanner,
...
PJ_DEF(void)
pj_scan_get_quotes(pj_scanner *scanner,
...
PJ_DEF(void)
pj_scan_get_n(pj_scanner *scanner,
...
/**
 * @brief 扫描字符串，指针指向下一行起始字符串
 * 
 */
PJ_DEF(void)
pj_scan_get_newline(pj_scanner *scanner)
{
...
PJ_DEF(void)
pj_scan_get_until(pj_scanner *scanner,
                  const pj_cis_t *spec, pj_str_t *out)
{
    register char *s = scanner->curptr;
...
PJ_DEF(void)
pj_scan_get_until_ch(pj_scanner *scanner,
...
PJ_DEF(void)
pj_scan_get_until_chr(pj_scanner *scanner,
...
PJ_DEF(void)
pj_scan_advance_n(pj_scanner *scanner,
...
PJ_DEF(int)
pj_scan_strcmp(pj_scanner *scanner, const char *s, int len)
{
...
PJ_DEF(int)
pj_scan_stricmp(pj_scanner *scanner, const char *s, int len)
{
    if (scanner->curptr + len > scanner->end)
    {
        pj_scan_syntax_err(scanner);
...
PJ_DEF(int)
pj_scan_stricmp_alnum(pj_scanner *scanner, const char *s,
                      int len)
{
    if (scanner->curptr + len > scanner->end)
    {
...
PJ_DEF(void)
pj_scan_save_state(const pj_scanner *scanner,
                   pj_scan_state *state)
{
...
PJ_DEF(void)
pj_scan_restore_state(pj_scanner *scanner,
                      pj_scan_state *state)
{
