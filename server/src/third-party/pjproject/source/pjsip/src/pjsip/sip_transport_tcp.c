 ...
 */
PJ_DEF(void)
pjsip_tcp_transport_cfg_default(pjsip_tcp_transport_cfg *cfg,
                                int af)
{
    ...
    pjsip_tpfactory **p_factory)
{
    ...
    listener->factory.type = cfg->af == pj_AF_INET() ? PJSIP_TRANSPORT_TCP : PJSIP_TRANSPORT_TCP6;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tcp_transport_start2(pjsip_endpoint *endpt,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tcp_transport_start(pjsip_endpoint *endpt,
                          const pj_sockaddr_in *local,
                          unsigned async_cnt,
                          pjsip_tpfactory **p_factory)
{
...
PJ_DEF(pj_sock_t)
pjsip_tcp_transport_get_socket(pjsip_transport *transport)
{
    struct tcp_transport *tcp = (struct tcp_transport *)transport;
...
PJ_DEF(pj_status_t)
pjsip_tcp_transport_restart(pjsip_tpfactory *factory,
                            const pj_sockaddr *local,
                            const pjsip_host_port *a_name)
{
    pj_status_t status = PJ_SUCCESS;
