 ...
 */
PJ_DEF(const char *)
pjsip_transport_get_type_name(pjsip_transport_type_e type)
{
	/* Return the name. */
 ...
 */
PJ_DEF(const char *)
pjsip_transport_get_type_desc(pjsip_transport_type_e type)
{
						  ...
						  const pjsip_msg *msg)
{
	...
	if (msg->type == PJSIP_REQUEST_MSG)
	{
		len = pj_ansi_snprintf(info_buf, sizeof(info_buf),
							   "[sp][message]Request msg %.*s/cseq=%d (%s)",
	...
	{
		len = pj_ansi_snprintf(info_buf, sizeof(info_buf),
							   "[sp][message]Response msg %d/%.*s/cseq=%d (%s)",
...
pjsip_transport_dec_ref(pjsip_transport *tp)
{
	 ...
	 */
	     ...
	     */
			...
			{
				delay.sec = (tp->dir == PJSIP_TP_DIR_OUTGOING) ? PJSIP_TRANSPORT_IDLE_TIME : PJSIP_TRANSPORT_SERVER_IDLE_TIME;
				delay.msec = 0;
			}
						  ...
						  pj_bool_t force)
{
	...
	if (state_cb)
	{
		...
		(*state_cb)(tp, (force ? PJSIP_TP_STATE_DISCONNECTED : PJSIP_TP_STATE_SHUTDOWN), &state_info);
	}
 ...
 * UDP传输，使用若发送对UDP包大小超过MTU IP层会自动分片，接收时IP层也会自动合并，然后交给应用层解析。因此应用层读到的一个包就是一个sip包
 * TCP传输，不会分片，但会粘包和拆包，需要sip协议进行重组。
 */
PJ_DEF(pj_ssize_t)
pjsip_tpmgr_receive_packet(pjsip_tpmgr *mgr,
						   pjsip_rx_data *rdata)
{
	PJ_LOG(3, (THIS_FILE, "[sp][socket][TM]receive packet begin"));
	...
	PJ_LOG(3, (THIS_FILE, "[sp][socket] last receive length:%d remaining length is:%d", tr->last_recv_len, remaining_len));
	 ...
	 */
		...
		if (p != current_pkt)
		{
			...
			if (mgr->tp_drop_data_cb)
			{
				...
				PJ_LOG(3, (THIS_FILE, "[sp][socket] drop data len:%d", dd.len));
	 ...
	 */
		...
		{
			PJ_LOG(3, (THIS_FILE, "[sp][tcp][socket] tcp check whole message has been received"));
 ...
 */
PJ_DEF(pjsip_tp_state_callback)
