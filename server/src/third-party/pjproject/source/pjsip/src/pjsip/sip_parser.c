...
PJ_DEF(const pjsip_parser_const_t *)
pjsip_parser_const(void)
{
    return &pconst;
}
...
PJ_DEF(void)
pjsip_concat_param_imp(pj_str_t *param, pj_pool_t *pool,
...
{
    PJ_LOG(3, (THIS_FILE, "[sp][parse] init sip parser"));
    pj_status_t status;
    ...
    // PJ_LOG(3, (THIS_FILE, "[sp][parse] cis_buf mask is:%d should be:1", cis_buf.use_mask));

    status = pj_cis_init(&cis_buf, &pconst.pjsip_ALPHA_SPEC);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    pj_cis_add_alpha(&pconst.pjsip_ALPHA_SPEC);

    // PJ_LOG(3, (THIS_FILE, "[sp][parse] cis_buf mask is:%d should be 3", cis_buf.use_mask));
    ...
    // PJ_LOG(3, (THIS_FILE, "[sp][parse] cis_buf mask is:%d should be 7", cis_buf.use_mask));

    status = pj_cis_init(&cis_buf, &pconst.pjsip_NOT_NEWLINE);
    PJ_ASSERT_RETURN(status == PJ_SUCCESS, status);
    // "\r" -- 13 ascii  "\n" -- 10 ascii
    // 以下两行是将cis_buf[10] cis_buf[13]第(1<<pconst.pjsip_NOT_NEWLINE->cis_id)位置0，其它元素的该位置1
    pj_cis_add_str(&pconst.pjsip_NOT_NEWLINE, "\r\n");
    pj_cis_invert(&pconst.pjsip_NOT_NEWLINE);

    // PJ_LOG(3, (THIS_FILE, "[sp][parse] cis_buf mask is:%d should be 15", cis_buf.use_mask));
    ...
    //TODO:fixme: DELETE test
    // PJ_LOG(3, (THIS_FILE, "[sp][parser] sip header handler length:%d", sizeof(handler)));
    // PJ_LOG(3, (THIS_FILE, "[sp][parser] sip header handler_rec size:%d", sizeof(handler_rec)));
    // PJ_LOG(3, (THIS_FILE, "[sp][parser] sip header handler array size:%d", sizeof(handler) / sizeof(handler_rec)));

    //handler 是按hash大小排序的有序集合
    for (size_t i = 0; i < handler_count; i++)
    {
        handler_rec *rec = &handler[i];
        if (rec != NULL)
        {
            // PJ_LOG(3, (THIS_FILE, "[sp][parser] sip header parser handler[%d] = %s", i, rec->hname));
            // PJ_LOG(3, (THIS_FILE, "[sp][parser] sip header parser handler[%d] hash= %d", i, rec->hname_hash));
        }
    }
 ...
 */
PJ_INLINE(int)
compare_handler(const handler_rec *r1,
                const char *name,
                pj_size_t name_len,
                pj_uint32_t hash)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_register_hdr_parser(const char *hname,
...
/* Register URI parser. */
PJ_DEF(pj_status_t)
pjsip_register_uri_parser(char *scheme,
...
PJ_DEF(pjsip_msg *)
pjsip_parse_msg(pj_pool_t *pool,
...
PJ_DEF(pjsip_msg *)
pjsip_parse_rdata(char *buf, pj_size_t size,
                  pjsip_rx_data *rdata)
{
    PJ_LOG(3, (THIS_FILE, "[sp][parse] receive sip packet parse:\n%s", buf));
    pj_scanner scanner;
    pjsip_parse_ctx context;
...
/* Determine if a message has been received.(TCP) */
PJ_DEF(pj_status_t)
pjsip_find_msg(const char *buf, pj_size_t size,
...
PJ_DEF(pjsip_uri *)
pjsip_parse_uri(pj_pool_t *pool,
                char *buf, pj_size_t size,
                unsigned option)
{
    pj_scanner scanner;
...
static pj_bool_t is_next_sip_version(pj_scanner *scanner)
{
    ...
    //遍历查找并截取从起始到第一个非'a'-'z' 'A'-'Z'的字符串并输出到pj_str_t sip
...
retry_parse:
	 ...
	 */
        ...
        {
            PJ_LOG(3, (THIS_FILE, "[sp][parse] start with 'sip/' is response message"));
            msg = pjsip_msg_create(pool, PJSIP_RESPONSE_MSG);
            int_parse_status_line(scanner, &msg->line.status);
        }
        else
        {
            PJ_LOG(3, (THIS_FILE, "[sp][parse] is request message"));
    ...
    parse_headers:
		 ...
		 */
                ...
                {
                    PJ_LOG(3, (THIS_FILE, "[sp][parse] header is 'Content-Type'"));
	 ...
	 */
        ...
        {
            PJ_LOG(3, (THIS_FILE, "[sp][parse] pasing content body by content_type"));
...
PJ_DEF(void)
pjsip_parse_param_imp(pj_scanner *scanner, pj_pool_t *pool,
                      pj_str_t *pname, pj_str_t *pvalue,
                      unsigned option)
{
...
PJ_DEF(void)
pjsip_parse_uri_param_imp(pj_scanner *scanner, pj_pool_t *pool,
                          pj_str_t *pname, pj_str_t *pvalue,
                          unsigned option)
{
...
static void *int_parse_sip_url(pj_scanner *scanner,
                               ...
                               pj_bool_t parse_params)
{
    PJ_LOG(3, (THIS_FILE, "[sp][parse] parse sip URI"));
    pj_str_t scheme;
    ...
    {
        PJ_LOG(3, (THIS_FILE, "[sp][parse] scheme is [sip]"));
        url = pjsip_sip_uri_create(pool, 0);
    }
    else if (parser_stricmp(scheme, pconst.pjsip_SIPS_STR) == 0)
    {
        PJ_LOG(3, (THIS_FILE, "[sp][parse] scheme is [sips]"));
...
/* Parse ending of header. */
PJ_DEF(void)
pjsip_parse_end_hdr_imp(pj_scanner *scanner)
{
...
PJ_DEF(void *)
pjsip_parse_hdr(pj_pool_t *pool, const pj_str_t *hname,
...
PJ_DEF(pj_status_t)
pjsip_parse_headers(pj_pool_t *pool, char *input,
...
retry_parse:
    PJ_TRY
    {
	     ...
	     */
            ...
            /* Get hname. */
            pj_scan_get(&scanner, &pconst.pjsip_TOKEN_SPEC, &hname);
            PJ_LOG(3, (THIS_FILE, "[sp][parse] parsing headers get Name:%s", hname.ptr));
            if (pj_scan_get_char(&scanner) != ':')
            {
                PJ_THROW(PJSIP_SYN_ERR_EXCEPTION);
            }
	     ...
	     */
            if (func)
            {
                PJ_LOG(3, (THIS_FILE, "[sp][parse] found handler for header name:%s", hname.ptr));
                hdr = (*func)(&ctx);
            }
            else
            {
                PJ_LOG(3, (THIS_FILE, "[sp][parse] use generic string parse handler for header name:%s", hname.ptr));
                hdr = parse_hdr_generic_string(&ctx);
                hdr->name = hdr->sname = hname;
            }
