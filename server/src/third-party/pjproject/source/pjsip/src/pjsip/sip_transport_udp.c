...
{
	PJ_LOG(3, (THIS_FILE, "[sp][socket][udp] read data length:%ld", bytes_read));
...
static pj_status_t start_async_read(struct udp_transport *tp)
{
	int i;
	pj_status_t status;
	PJ_LOG(3, (THIS_FILE, "[sp][socket]start async read socket %d from ioqueue", tp->sock));
...
{
	PJ_LOG(3, (THIS_FILE, "[sp][socket] socket:%d type:%d transport attach with host:%s port:%d", sock, type, a_name->host, a_name->port));
	pj_pool_t *pool;
	...
	tp->base.key.rem_addr.addr.sa_family = (pj_uint16_t)((type & PJSIP_TRANSPORT_IPV6) ? pj_AF_INET6() : pj_AF_INET());
	 ...
	 * 将该udptrasnport 保存到TransportManager的hash表中
	 */
	...
	PJ_LOG(3, (THIS_FILE, "[sp][socket] async read data account:%d", async_cnt));
	...
	PJ_LOG(4, (tp->base.obj_name,
			   "[sp][socket]SIP %s started, published address is %s%.*s%s:%d",
...
PJ_DEF(pj_status_t)
pjsip_udp_transport_attach(pjsip_endpoint *endpt,
						   pj_sock_t sock,
						   const pjsip_host_port *a_name,
						   unsigned async_cnt,
						   pjsip_transport **p_transport)
...
PJ_DEF(pj_status_t)
pjsip_udp_transport_attach2(pjsip_endpoint *endpt,
							pjsip_transport_type_e type,
							pj_sock_t sock,
							const pjsip_host_port *a_name,
							unsigned async_cnt,
 ...
 */
PJ_DEF(void)
pjsip_udp_transport_cfg_default(pjsip_udp_transport_cfg *cfg,
								int af)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_start(pjsip_endpoint *endpt,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_pause(pjsip_transport *transport,
						  unsigned option)
{
	struct udp_transport *tp;
	unsigned i;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_udp_transport_restart(pjsip_transport *transport,
							unsigned option,
							pj_sock_t sock,
...
PJ_DEF(pj_status_t)
pjsip_udp_transport_restart2(pjsip_transport *transport,
							 unsigned option,
							 pj_sock_t sock,
