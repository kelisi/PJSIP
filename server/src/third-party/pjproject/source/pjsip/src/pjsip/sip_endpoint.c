...
pjsip_endpt_register_module(pjsip_endpoint *endpt,
                            pjsip_module *mod)
{
    PJ_LOG(3, (THIS_FILE, "[sp][endpoint][module] register module:%s with priority:%d", mod->name.ptr, mod->priority));
                   ...
                   pjsip_endpoint **p_endpt)
{
    ...
    *p_endpt = NULL;
    ...
    /* Create ioqueue. */
    PJ_LOG(3, (THIS_FILE, "[io]create ioqueue with max transports:%d", PJSIP_MAX_TRANSPORTS));
                           ...
                           unsigned *p_count)
{
    ...
    PJ_LOG(6, (THIS_FILE, "[CORE][TH][EVENT]pjsip_endpt_handle_events()"));
    ...
    do
    {
        // PJ_LOG(3, (THIS_FILE,"[CORE][SP][TH][EVENT][IO] poll form ioqueue"));
        c = pj_ioqueue_poll(endpt->ioqueue, &timeout);
        // PJ_LOG(3, (THIS_FILE, "[CORE][TH][EVENT][IO] polling from ioqueue event counts:%d!", c));
                            ...
                            pj_bool_t *p_handled)
{
    ...
    if (!p->silent)
    {
        PJ_LOG(5, (THIS_FILE, "[sp][message]Distributing rdata to modules: %s",
...
static void endpt_on_rx_msg(pjsip_endpoint *endpt,
                            ...
                            pjsip_rx_data *rdata)
{
    PJ_LOG(3, (THIS_FILE, "[sp][message] receive sip socket message"));
    ...
    PJ_LOG(3, (THIS_FILE, "[sp][message]Processing incoming message: %s",
               pjsip_rx_data_get_info(rdata)));
    pj_log_push_indent();
