 ...
 */
PJ_DEF(const char *)
pjsip_tsx_state_str(pjsip_tsx_state_e state)
{
 ...
 */
PJ_DEF(const char *)
pjsip_role_name(pjsip_role_e role)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_create_key(pj_pool_t *pool, pj_str_t *key,
					 pjsip_role_e role,
					 const pjsip_method *method,
					 const pjsip_rx_data *rdata)
{
	...
	{
		PJ_LOG(3, (THIS_FILE, "[sp][ua][tsx] rfc3261 create key with role[%c] method[%s]", ((role == PJSIP_ROLE_UAC) ? 'c' : 's'), method->name.ptr));
	...
	{
		PJ_LOG(3, (THIS_FILE, "[sp][ua][tsx] rfc2563 create key with role[%c] method[%s]", ((role == PJSIP_ROLE_UAC) ? 'c' : 's'), method->name.ptr));
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_layer_init_module(pjsip_endpoint *endpt)
{
	pj_pool_t *pool;
	pj_status_t status;
 ...
 */
PJ_DEF(unsigned)
pjsip_tsx_layer_get_tsx_count(void)
{
	unsigned count;
								   ...
								   pj_bool_t add_ref)
{
	...
	TSX_TRACE_((THIS_FILE,
				"[sp][ua][tsx]Finding tsx with hkey=0x%p and key=%.*s: found %p",
...
pjsip_tsx_layer_find_tsx(const pj_str_t *key,
						 pj_bool_t lock)
{
...
PJ_DEF(pjsip_transaction *)
pjsip_tsx_layer_find_tsx2(const pj_str_t *key,
						  pj_bool_t add_ref)
{
...
static pj_bool_t mod_tsx_layer_on_rx_request(pjsip_rx_data *rdata)
{
	TSX_TRACE_((THIS_FILE, "[TM][TS] transaction layer receive request!"));
	pj_str_t key;
	pj_uint32_t hval = 0;
	pjsip_transaction *tsx;
	...
	TSX_TRACE_((THIS_FILE,
				"[TM][TS]Finding tsx for request, hkey=0x%p and key=%.*s, found %p",
				hval, key.slen, key.ptr, tsx));
	...
	TSX_TRACE_((THIS_FILE, "[TM][TS]simulate race condition!"));
	/* Simulate race condition! */
	PJ_RACE_ME(5);
 ...
 */
PJ_DEF(pjsip_transaction *)
pjsip_rdata_get_tsx(pjsip_rx_data *rdata)
{
 ...
 */
PJ_DEF(void)
pjsip_tsx_layer_dump(pj_bool_t detail)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_create_uac(pjsip_module *tsx_user,
					 pjsip_tx_data *tdata,
					 pjsip_transaction **p_tsx)
{
...
PJ_DEF(pj_status_t)
pjsip_tsx_create_uac2(pjsip_module *tsx_user,
					  pjsip_tx_data *tdata,
					  pj_grp_lock_t *grp_lock,
					  pjsip_transaction **p_tsx)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_create_uas(pjsip_module *tsx_user,
					 pjsip_rx_data *rdata,
					 pjsip_transaction **p_tsx)
{
...
PJ_DEF(pj_status_t)
pjsip_tsx_create_uas2(pjsip_module *tsx_user,
					  pjsip_rx_data *rdata,
					  pj_grp_lock_t *grp_lock,
					  pjsip_transaction **p_tsx)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_set_transport(pjsip_transaction *tsx,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_terminate(pjsip_transaction *tsx, int code)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_set_timeout(pjsip_transaction *tsx,
					  unsigned millisec)
{
	pj_time_val timeout;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_send_msg(pjsip_transaction *tsx,
				   pjsip_tx_data *tdata)
{
	pjsip_event event;
	pj_status_t status;
 ...
 */
PJ_DEF(void)
pjsip_tsx_recv_msg(pjsip_transaction *tsx,
				   pjsip_rx_data *rdata)
{
	pjsip_event event;

	PJ_LOG(5, (tsx->obj_name, "[TM][TS] pjsip_tsx_recv_msg Incoming %s in state %s",
 ...
 */
PJ_DEF(pj_status_t)
pjsip_tsx_retransmit_no_state(pjsip_transaction *tsx,
							  pjsip_tx_data *tdata)
{
	pj_status_t status;
