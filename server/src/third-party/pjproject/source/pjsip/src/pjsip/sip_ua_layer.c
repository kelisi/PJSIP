 ...
 */
PJ_DEF(pj_status_t)
pjsip_ua_init_module(pjsip_endpoint *endpt,
...
pjsip_ua_get_endpt(pjsip_user_agent *ua)
{
	PJ_UNUSED_ARG(ua);
 ...
 */
PJ_DEF(pj_status_t)
pjsip_ua_register_dlg(pjsip_user_agent *ua,
					  pjsip_dialog *dlg)
{
	/* Sanity check. */
...
PJ_DEF(pj_status_t)
pjsip_ua_unregister_dlg(pjsip_user_agent *ua,
						pjsip_dialog *dlg)
{
	struct dlg_set *dlg_set;
	pjsip_dialog *d;
...
PJ_DEF(pjsip_dialog *)
pjsip_rdata_get_dlg(pjsip_rx_data *rdata)
{
	return (pjsip_dialog *)rdata->endpt_info.mod_data[mod_ua.mod.id];
}

PJ_DEF(pjsip_dialog *)
pjsip_tdata_get_dlg(pjsip_tx_data *tdata)
{
	return (pjsip_dialog *)tdata->mod_data[mod_ua.mod.id];
}

PJ_DEF(pjsip_dialog *)
pjsip_tsx_get_dlg(pjsip_transaction *tsx)
{
 ...
 */
PJ_DEF(unsigned)
pjsip_ua_get_dlg_set_count(void)
{
	unsigned count;
 ...
 */
PJ_DEF(pjsip_dialog *)
pjsip_ua_find_dialog(const pj_str_t *call_id,
					 const pj_str_t *local_tag,
...
static struct dlg_set *find_dlg_set_for_msg(pjsip_rx_data *rdata)
{
	 ...
	 */
		...
		PJ_LOG(3, (THIS_FILE, "[sp][ua] method id is [PJSIP_CANCEL_METHOD] role is [%c]", (role == PJSIP_ROLE_UAC ? 'c' : 's')));
		...
		if (tsx)
		{
			PJ_LOG(3, (THIS_FILE, "[sp][ua] found tsx for INVITE transaction"));
			dlg = (pjsip_dialog *)tsx->mod_data[mod_ua.mod.id];
		...
		{
			PJ_LOG(3, (THIS_FILE, "[sp][ua] not found tsx for INVITE transaction"));
			return NULL;
		}
	}
	else
	{
		PJ_LOG(3, (THIS_FILE, "[sp][ua] method id is %d", rdata->msg_info.cseq->method.id));
		pj_str_t *tag;
		struct dlg_set *dlg_set;
		...
		PJ_LOG(3, (THIS_FILE, "[sp][ua] tag is %s value from [%s]", tag->ptr, ((rdata->msg_info.msg->type == PJSIP_REQUEST_MSG) ? "To" : "From")));
		/* Lookup the dialog set. */
		dlg_set = (struct dlg_set *)
...
static pj_bool_t mod_ua_on_rx_request(pjsip_rx_data *rdata)
{
	PJ_LOG(3, (THIS_FILE, "[sp][ua][request] received request on ua layer method id is:%d", rdata->msg_info.msg->line.req.method.id));
	struct dlg_set *dlg_set;
	pj_str_t *from_tag;
	pjsip_dialog *dlg;
	pj_status_t status;
...
retry_on_deadlock:
	...
	if (dlg_set == NULL)
	{
		PJ_LOG(3, (THIS_FILE, "[sp][ua][request] dialog set not found :response with 481"));
						 ...
						 pjsip_dialog *dlg, char *buf, pj_size_t size)
{
	...
	len = pj_ansi_snprintf(buf, size, "%s[%s]  %s",
						   title,
						   (dlg->state == PJSIP_DIALOG_STATE_NULL ? " - " : "est"),
						   userinfo);
 ...
 */
PJ_DEF(void)
pjsip_ua_dump(pj_bool_t detail)
{
#if PJ_LOG_MAX_LEVEL >= 3
