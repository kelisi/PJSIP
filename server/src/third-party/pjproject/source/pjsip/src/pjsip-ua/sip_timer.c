...
static void timer_cb(pj_timer_heap_t *timer_heap, struct pj_timer_entry *entry)
{
	 ...
	 */
		...
		if ((!inv->timer->use_update && (inv->invite_tsx != NULL ||
	...
	if (status != PJ_SUCCESS)
	{
		PJ_PERROR(2, (obj_name, status,
					  ...
					  ((as_refresher && entry_id != REFRESHER_EXPIRE_TIMER_ID) ? "refreshing" : "terminating")));
	}
}
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_init_module(pjsip_endpoint *endpt)
{
	pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_setting_default(pjsip_timer_setting *setting)
{
 ...
 */
PJ_DEF(pjsip_min_se_hdr *)
pjsip_min_se_hdr_create(pj_pool_t *pool)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_update_req(pjsip_inv_session *inv,
					   pjsip_tx_data *tdata)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_process_resp(pjsip_inv_session *inv,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_process_req(pjsip_inv_session *inv,
						const pjsip_rx_data *rdata,
						pjsip_status_code *st_code)
{
	pjsip_min_se_hdr *min_se_hdr;
	 ...
	 */
	     ...
	     */
			...
			inv->timer->refresher = (uac_supports_timer && se_hdr) ? TR_UAC : TR_UAS;
		}
	}
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_update_resp(pjsip_inv_session *inv,
						pjsip_tx_data *tdata)
{
	pjsip_msg *msg;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_timer_end_session(pjsip_inv_session *inv)
{
	PJ_ASSERT_RETURN(inv, PJ_EINVAL);
