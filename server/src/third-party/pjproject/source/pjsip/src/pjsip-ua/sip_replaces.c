...
PJ_DEF(pjsip_replaces_hdr *)
pjsip_replaces_hdr_create(pj_pool_t *pool)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_replaces_init_module(pjsip_endpoint *endpt)
{
    pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_replaces_verify_request(pjsip_rx_data *rdata,
                              pjsip_dialog **p_dlg,
                              pj_bool_t lock_dlg,
...
on_return:
    ...
    if (code != 200)
    {
        ...
        if (p_tdata)
        {
            ...
            if (warn_text)
            {
                PJ_LOG(3, (THIS_FILE, "[sp][invite] found Replaces header warn:%s", warn_text));
