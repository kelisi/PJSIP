...
static pj_status_t mod_inv_load(pjsip_endpoint *endpt)
{
	pj_str_t allowed[] = {{"INVITE", 6}, {"ACK", 3}, {"BYE", 3}, {"CANCEL", 6}, {"UPDATE", 6}};
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_add_ref(pjsip_inv_session *inv)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_dec_ref(pjsip_inv_session *inv)
{
														 ...
														 pjsip_transaction *tsx)
{
	...
	neg_state = inv->neg ? pjmedia_sdp_neg_get_state(inv->neg) : PJMEDIA_SDP_NEG_STATE_NULL;
...
static pj_bool_t mod_inv_on_rx_request(pjsip_rx_data *rdata)
{
	PJ_LOG(3, (THIS_FILE, "[sp][invite] receive request"));
	pjsip_method *method;
	pjsip_dialog *dlg;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_usage_init(pjsip_endpoint *endpt,
 ...
 */
PJ_DEF(pjsip_inv_session *)
pjsip_dlg_get_inv_session(pjsip_dialog *dlg)
{
 ...
 */
PJ_DEF(const char *)
pjsip_inv_state_name(pjsip_inv_state state)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_create_uac(pjsip_dialog *dlg,
					 const pjmedia_sdp_session *local_sdp,
					 unsigned options,
					 pjsip_inv_session **p_inv)
{
	PJ_LOG(3, (THIS_FILE, "[sp][invite] create UAC invite session"));
	pjsip_inv_session *inv;
	pj_status_t status;
...
pjsip_rdata_get_sdp_info(pjsip_rx_data *rdata)
{
	...
	{
		pj_status_t status;
		PJ_LOG(3, (THIS_FILE, "[sp][call] parsing sdp info form INVITE request -- begin"));
		status = pjmedia_sdp_parse(rdata->tp_info.pool,
								   sdp_info->body.ptr,
								   sdp_info->body.slen,
								   &sdp_info->sdp);
		PJ_LOG(3, (THIS_FILE, "[sp][call] parsing sdp info form INVITE request -- end"));
		if (status == PJ_SUCCESS)
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_verify_request3(pjsip_rx_data *rdata,
						  pj_pool_t *tmp_pool,
						  unsigned *options,
...
on_return:
	PJ_LOG(3, (THIS_FILE, "[sp][call] verify INVITE request code:%d", code));
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_verify_request2(pjsip_rx_data *rdata,
						  unsigned *options,
						  const pjmedia_sdp_session *r_sdp,
						  const pjmedia_sdp_session *l_sdp,
						  pjsip_dialog *dlg,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_verify_request(pjsip_rx_data *rdata,
						 unsigned *options,
						 const pjmedia_sdp_session *l_sdp,
						 pjsip_dialog *dlg,
						 pjsip_endpoint *endpt,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_create_uas(pjsip_dialog *dlg,
					 pjsip_rx_data *rdata,
					 const pjmedia_sdp_session *local_sdp,
					 unsigned options,
					 pjsip_inv_session **p_inv)
{
	...
	pjsip_dlg_dec_lock(dlg);
	*p_inv = inv;
	PJ_LOG(5, (THIS_FILE, "[sp][call] inv->obj_name:%s UAS invite session created for dialog %s",
			   inv->obj_name,
			   dlg->obj_name));
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_terminate(pjsip_inv_session *inv,
					int st_code,
					pj_bool_t notify)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_uac_restart(pjsip_inv_session *inv,
					  pj_bool_t new_offer)
{
	PJ_ASSERT_RETURN(inv, PJ_EINVAL);
...
PJ_DEF(pj_status_t)
pjsip_create_sdp_body(pj_pool_t *pool,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_invite(pjsip_inv_session *inv,
				 pjsip_tx_data **p_tdata)
{
	pjsip_tx_data *tdata;
	const pjsip_hdr *hdr;
...
static pj_status_t inv_negotiate_sdp(pjsip_inv_session *inv)
{
	PJ_LOG(3, (THIS_FILE, "[call][sdp] initiate SDP negotiation"));
	pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_initial_answer(pjsip_inv_session *inv,
						 pjsip_rx_data *rdata,
						 int st_code,
						 ...
						 pjsip_tx_data **p_tdata)
{
	...
	PJ_LOG(5, (THIS_FILE, "[sp][call][inv]Initial answer %s",
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_answer(pjsip_inv_session *inv,
				 int st_code,
				 const pj_str_t *st_text,
				 const pjmedia_sdp_session *local_sdp,
				 pjsip_tx_data **p_tdata)
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_set_local_sdp(pjsip_inv_session *inv,
						const pjmedia_sdp_session *sdp)
{
	const pjmedia_sdp_session *offer;
	pj_status_t status;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_end_session(pjsip_inv_session *inv,
					  int st_code,
					  const pj_str_t *st_text,
					  pjsip_tx_data **p_tdata)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_cancel_reinvite(pjsip_inv_session *inv,
						  pjsip_tx_data **p_tdata)
{
	pjsip_tx_data *tdata;
	pj_status_t status;
...
PJ_DEF(pj_status_t)
pjsip_inv_process_redirect(pjsip_inv_session *inv,
						   pjsip_redirect_op op,
						   pjsip_event *e)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_reinvite(pjsip_inv_session *inv,
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_update(pjsip_inv_session *inv,
				 const pj_str_t *new_contact,
				 const pjmedia_sdp_session *offer,
				 pjsip_tx_data **p_tdata)
{
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_create_ack(pjsip_inv_session *inv,
					 int cseq,
					 pjsip_tx_data **p_tdata)
{
	const pjmedia_sdp_session *sdp = NULL;
 ...
 */
PJ_DEF(pj_status_t)
pjsip_inv_send_msg(pjsip_inv_session *inv,
				   pjsip_tx_data *tdata)
{
	pj_status_t status;
	 ...
	 * request.
	 */
		PJ_ASSERT_RETURN((cseq = (pjsip_cseq_hdr *)pjsip_msg_find_hdr(tdata->msg, PJSIP_H_CSEQ, NULL)) != NULL && (cseq->cseq == inv->invite_tsx->cseq),
						 PJ_EINVALIDOP);
