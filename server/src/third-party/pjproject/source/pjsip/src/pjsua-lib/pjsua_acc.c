 ...
 */
PJ_DEF(pj_status_t)
pjsua_acc_add(const pjsua_acc_config *cfg,
			  ...
			  pjsua_acc_id *p_acc_id)
{
	...
	PJ_LOG(4, (THIS_FILE, "[account]Adding account: id=%.*s",
...
done:
{
	...
	len = acc->contact.slen + acc->cfg.reg_contact_params.slen +
		  (need_outbound ? (acc->rfc5626_instprm.slen + acc->rfc5626_regprm.slen) : 0);
...
on_return:
	...
	PJ_LOG(4, (THIS_FILE, "SIP outbound status for acc %d is %s",
			   acc->index, (acc->rfc5626_status == OUTBOUND_ACTIVE ? "active" : "not active")));
}
...
static void schedule_reregistration(pjsua_acc *acc)
{
	...
	delay.sec = acc->auto_rereg.attempt_cnt ? acc->cfg.reg_retry_interval : acc->cfg.reg_first_retry_interval;
	delay.msec = 0;
