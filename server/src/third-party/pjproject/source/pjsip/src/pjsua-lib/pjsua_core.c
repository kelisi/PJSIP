...
pjsua_logging_config_default(pjsua_logging_config *cfg)
{
	...
	cfg->decor = PJ_LOG_HAS_SENDER | PJ_LOG_HAS_TIME |
				 ...
				 PJ_LOG_HAS_INDENT | PJ_LOG_HAS_COLOR;
...
pjsua_transport_config_default(pjsua_transport_config *cfg)
{
	...
	PJ_LOG(3, (THIS_FILE, "[core][sp][cfg]config transport with default:\n \
	 tls_opt->reuse_addr = %d;\n \
	 tls_opt->qos_type = %d;\n \
	 tls_opt->qos_ignore_error = %d;\n \
	 tls_opt->sockopt_ignore_error = %d;\n \
	 tls_opt->proto = %d;",
			   cfg->tls_setting.reuse_addr,
			   cfg->tls_setting.qos_type,
			   cfg->tls_setting.qos_ignore_error,
			   cfg->tls_setting.sockopt_ignore_error,
			   cfg->tls_setting.proto));
}
...
{
	PJ_LOG(3, (THIS_FILE, "[sp][core] receive requests"));
	...
	{
		PJ_LOG(3, (THIS_FILE, "[sp][core] receive incoming INVITE request"));
 ...
 */
PJ_DEF(pj_status_t)
pjsua_reconfigure_logging(const pjsua_logging_config *cfg)
{
	pj_status_t status;
...
{
	PJ_LOG(3, (THIS_FILE, "[CORE][TH] thread %s working!", pj_thread_get_name(pj_thread_this())));
	enum
	{
		TIMEOUT = 10
	};
...
static void init_random_seed(void)
{
	...
	PJ_LOG(3, (THIS_FILE, "[init][random seed] host name is:%s len:%d", hostname->ptr, hostname->slen));
		   ...
		   const pjsua_media_config *media_cfg)
{
	...
	{
		PJ_LOG(3, (THIS_FILE, "[CORE] start worker thread count:%d", pjsua_var.ua_cfg.thread_cnt));
		unsigned ii;
		...
		for (ii = 0; ii < pjsua_var.ua_cfg.thread_cnt; ++ii)
		{
			...
			PJ_LOG(3, (THIS_FILE, "[CORE][TH] create worker thread with name:%s", tname));
#if PJSUA_SEPARATE_WORKER_FOR_TIMER
			if (ii == 0)
			{
		...
		PJ_LOG(4, (THIS_FILE, "[CORE][TH]%d SIP worker threads created",
...
#else
	// PJ_LOG(3, (THIS_FILE, "[CORE][TH] polling event from ioqueue!"));
	unsigned count = 0;
	pj_time_val tv;
	pj_status_t status;
...
{
	PJ_LOG(3, (THIS_FILE, "[core][sp] create sip udp socket"));
	...
	/* Bind socket */
	PJ_LOG(3, (THIS_FILE, "[core][sp][socket] bind socket:%d ", sock));
	...
	*p_sock = sock;
	...
	PJ_LOG(4, (THIS_FILE, "[core][sp][socket]SIP UDP socket reachable at %s:%d",
			   addr_string(p_pub_addr),
			   (int)pj_sockaddr_get_port(p_pub_addr)));
...
{
	PJ_LOG(3, (THIS_FILE, "[core][sp] create sip transport %d id %d", type, p_id));
	pjsip_transport *tp;
	unsigned id;
	pj_status_t status;
	...
	{
		status = PJ_ETOOMANY;
		pjsua_perror(THIS_FILE, "[core][sp]Error creating transport", status);
		goto on_return;
	}
	...
	{
		PJ_LOG(3, (THIS_FILE, "[core][sp] create udp transport"));
		/*
	 ...
	 */
		...
		if (cfg->public_addr.slen)
		{
			...
			{
				pjsua_perror(THIS_FILE,
							 "[core][sp]Unable to resolve transport public address",
							 status);
				goto on_return;
			}
		}
					  ...
					  pj_bool_t force)
{
	...
	if (force)
	{
	     ...
	     */
			status = (*pjsua_var.tpdata[id].data.factory->destroy)(pjsua_var.tpdata[id].data.factory);
			if (status != PJ_SUCCESS)
				return status;
	 ...
	 */
		...
		case PJSIP_TRANSPORT_TLS:
		case PJSIP_TRANSPORT_TCP:
			return (*pjsua_var.tpdata[id].data.factory->destroy)(pjsua_var.tpdata[id].data.factory);
		default:
			return PJ_EINVAL;
		}
	}
 ...
 */
PJ_DEF(pj_status_t)
pjsua_verify_url(const char *c_url)
{
	pjsip_uri *p;
	pj_pool_t *pool;
	char *url;
 ...
 */
PJ_DEF(pj_status_t)
pjsua_verify_sip_url(const char *c_url)
{
	pjsip_uri *p;
	pj_pool_t *pool;
	char *url;
...
PJ_DEF(pj_status_t)
pjsua_handle_ip_change(const pjsua_ip_change_param *param)
{
	pj_status_t status = PJ_SUCCESS;
	int i = 0;
