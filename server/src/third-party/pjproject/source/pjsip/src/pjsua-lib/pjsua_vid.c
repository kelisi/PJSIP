...
pj_status_t pjsua_vid_subsys_init(void)
{
	...
	PJ_LOG(4, (THIS_FILE, "[video]Initializing video subsystem.."));
	pj_log_push_indent();
	...
	if (status != PJ_SUCCESS)
	{
		PJ_PERROR(1, (THIS_FILE, status,
					  "[video]Error creating PJMEDIA video format manager"));
		goto on_error;
	}
	...
	{
		PJ_PERROR(1, (THIS_FILE, status,
					  "[video]Error creating PJMEDIA converter manager"));
		goto on_error;
	}
	...
	{
		PJ_PERROR(1, (THIS_FILE, status,
					  "[video]Error creating PJMEDIA video codec manager"));
		goto on_error;
	}
...
PJ_DEF(void)
pjsua_vid_win_reset(pjsua_vid_win_id wid)
{
	pjsua_vid_win *w = &pjsua_var.win[wid];
