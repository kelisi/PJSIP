										...
										pjmedia_sock_info *skinfo)
{
	...
	for (i = 0; i < RTP_RETRY; ++i, acc->next_rtp_port += 2)
	{
		...
		/* Create RTP socket. */
		PJ_LOG(3, (THIS_FILE, "[call][RTP][socket] create RTP socket in UDP"));
		...
		PJ_LOG(3, (THIS_FILE, "[call][RTP][socket] bind RTP socket to port:%d in UDP", acc->next_rtp_port));

		/* Create RTCP socket. */
		PJ_LOG(3, (THIS_FILE, "[call][RTCP][socket] create RTCP socket in UDP"));
		...
		PJ_LOG(3, (THIS_FILE, "[call][RTCP][socket] bind RTCP socket to port:%d in UDP", acc->next_rtp_port + 1));
									 ...
									 pjsua_med_tp_state_cb cb)
{
	...
	PJ_LOG(4, (THIS_FILE, "[call][media]Call %d: %sinitializing media..",
			   call_id, (reinit ? "re-" : "")));
									   ...
									   const pjmedia_sdp_session *remote_sdp)
{
	...
	PJ_LOG(4, (THIS_FILE, "[call][media]Call %d: updating media..", call_id));
	pj_log_push_indent();
