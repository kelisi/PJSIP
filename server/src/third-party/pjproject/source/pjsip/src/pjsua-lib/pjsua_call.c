...
{
	PJ_LOG(3, (THIS_FILE, "[sp][call] reset call descriptor with id:%d", id));
...
{
	PJ_LOG(3, (THIS_FILE, "[sp][call] fixme>?"));
								   ...
								   const pjmedia_sdp_session *sdp)
{
	...
	PJ_LOG(5, (THIS_FILE, "[sp][call][sdp][nat]Call %d: remote NAT type is %d (%s)", call->index,
...
{
	PJ_LOG(3, (THIS_FILE, "[sp][core][call] handle incoming INVITE request"));
	pj_str_t contact;
	...
	PJ_LOG(4, (THIS_FILE, "[sp][call]Incoming %s", rdata->msg_info.info));
	pj_log_push_indent();
	...
	if (call_id == PJSUA_INVALID_ID)
	{
		pjsip_endpt_respond_stateless(pjsua_var.endpt, rdata,
									  ...
									  NULL, NULL);
		PJ_LOG(2, (THIS_FILE,
				   "[sp][call]Unable to accept incoming call (too many calls)"));
		goto on_return;
	}
	...
	PJ_LOG(3, (THIS_FILE, "[sp][call] create random string to call->cname_buf:%s", call->cname_buf));
	...
	{
		PJ_LOG(3, (THIS_FILE, "[sp][call] parsing sdp_info from incming INVITE"));
	...
	if (pjsua_var.acc[acc_id].contact.slen)
	{
		...
		PJ_LOG(3, (THIS_FILE, "[sp][call] get contact %s from account struct", contact.ptr));
	}
	else
	{
		PJ_LOG(3, (THIS_FILE, "[sp][call] create uas contact from account struct"));
	...
	{
		PJ_LOG(3, (THIS_FILE, "[sp][call] sdp not null || replaced_dlg not null init media channel "));
